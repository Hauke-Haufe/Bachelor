...
void CustomIntegrateCPU
    ...
    if (!block_value_map.Contains("tsdf") ||
        !block_value_map.Contains("weight")||
        !block_value_map.Contains("label")) {
        open3d::utility::LogError(
                "TSDF and/or weight and/or label not allocated in blocks, please implement "
                "customized integration.");
    }
    tsdf_t* tsdf_base_ptr = block_value_map.at("tsdf").GetDataPtr<tsdf_t>();
    weight_t* weight_base_ptr =
...
#if defined(__CUDACC__)
void ExtractSemanticPointCloudCUDA
#else
void ExtractSemanticPointCloudCPU
#endif
        (const core::Tensor& indices,
         const core::Tensor& nb_indices,
         const core::Tensor& nb_masks,
    ...
    const weight_t* weight_base_ptr =
            block_value_map.at("weight").GetDataPtr<weight_t>();
    const label_t* label_base_ptr = 
            block_value_map.at("label").GetDataPtr<label_t>();
    const color_t* color_base_ptr = nullptr;
    if (block_value_map.Contains("color")) {
        color_base_ptr = block_value_map.at("color").GetDataPtr<color_t>();
    }
    ...
    // Labels
    ArrayIndexer label_indexer;
    labels = core::Tensor({valid_size, 1}, core::UInt8, device);
    label_indexer = ArrayIndexer(labels, 1);
    // This pass extracts exact surface points.
    ...
    core::ParallelFor(device, n, [=] OPEN3D_DEVICE(index_t workload_idx) {
        ...
        for (index_t i = 0; i < 3; ++i) {
            ...
            if (weight_i > weight_threshold && tsdf_i * tsdf_o < 0) {
                ...
                uint8_t* label_ptr = label_indexer.GetDataPtr<uint8_t>(idx);

                const uint8_t label_o = label_base_ptr[linear_idx];
                const uint8_t label_i = label_base_ptr[linear_idx_i];
                *label_ptr = (weight_o > weight_i) ? label_o : label_i;
