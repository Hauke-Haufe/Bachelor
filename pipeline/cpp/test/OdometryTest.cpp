...
void Time(SubDataset data, t::pipelines::odometry::Method method, MaskMethod m_mehtod, core::Device  device){

    auto dataset = Tum_dataset(data);
    
    double hole_time = 0.0;
    std::vector<double> time;
    time.resize(dataset.get_size());
    ...
    for(int i = 0; i< dataset.get_size() -1 ; i++){
        ...
        //Odometry Functions dont get timed directly but all should have the same overhead 
...
int main() {
    ...
    std::ifstream config_file("/home/hauke/code/Beachlor/pipeline/cpp/test/configs/odometry_config.json");
