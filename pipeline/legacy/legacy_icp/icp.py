    ...
    def sequential_registration(pcds, config, transforms = None):
        ...
        for i in range(1,len(pcds)-1):
            ...
            if transforms is None:
                registration, info = pairwise_registration(pcds[i-1], pcds[i], config)
            else:
                registration, info = pairwise_registration(pcds[i-1], pcds[i], config, transforms[i-1])
