...
#the Atrous Convolution Part in ASPP
...
#the seperable Atrous Convolution Part in ASPP
...
#the Pooling Part of the ASPP Module
class ASPPPooling(nn.Sequential):
    def __init__(self, in_channels, out_channels):
        super(ASPPPooling, self).__init__(
            nn.AdaptiveAvgPool2d(1), #pooling layer
            nn.Conv2d(in_channels, out_channels, 1, bias=False), #Pointwise Conv
...
class ASPP(nn.Module):
    def __init__(self, in_channels, atrous_rates, seperable = False):
        ...
        self.project = nn.Sequential(
            nn.Conv2d(5 * out_channels, out_channels, 1, bias=False), #Pointwise Conv
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
            nn.Dropout(0.1),)
