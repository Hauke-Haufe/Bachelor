...
from util.cross_validation import Options
...
class Evaluation:
    ...
    def __init__(self, crossvalidation_root):
        ...
        for fold in self.root.iterdir():
            study_path = (fold / "optuna_study.db")
            if fold.is_dir() and study_path.is_file():

                study = optuna.load_study(study_name=f"fold_optimization",
                                storage=f"sqlite:///{fold}/optuna_study.db")
                self.studies[int(str(fold.name))] = study.trials_dataframe()
    ...
    def train_best_config_fold(self, fold):
        ...
        with open( self.root / f"{fold}"/ "best/config.yaml", "w") as f:
